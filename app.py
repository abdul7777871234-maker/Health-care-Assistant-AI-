...
<div class="hero-box">
    ...
    <h1 style="color: #f3f4f6; font-weight: 800; margin: 0; font-size: 2.2rem;">
...
with q1:
    ...
    if st.button("📄 Blood test basics", use_container_width=True):
        st.session_state.messages.append({
            "role": "user",
            "content": "Can you explain the basics of a standard blood test?"
        })
        st.rerun()
    ...
    if st.button("🔍 Causes of fatigue", use_container_width=True):
        st.session_state.messages.append({
            "role": "user",
            "content": "What are the common causes of chronic fatigue?"
        })
        st.rerun()
...
with q2:
    ...
    if st.button("🥗 Healthy diet", use_container_width=True):
        st.session_state.messages.append({
            "role": "user",
            "content": "Give me tips for maintaining a healthy balanced diet."
        })
        st.rerun()
    ...
    if st.button("💊 Medication side effects", use_container_width=True):
        st.session_state.messages.append({
            "role": "user",
            "content": "How can I check or manage common medication side effects?"
        })
        st.rerun()
...
st.markdown(
    '<div class="notice-box">🔒 <b>Ephemeral Data Processing</b> — '
    'any medical document you attach is deleted automatically within 1 hour. '
    'Your chat history stays saved in this conversation.</div>',
    unsafe_allow_html=True
)

st.markdown(
    '<div class="disclaimer-box">⚠️ <b>Important:</b> This AI Healthcare Assistant '
    'provides general informational support only. It does not diagnose medical '
    'conditions, prescribe treatment, or replace advice from a qualified '
